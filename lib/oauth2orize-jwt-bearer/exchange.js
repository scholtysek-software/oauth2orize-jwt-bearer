...
var utils = require('./utils')
  ...
  , LinkingError = require('./errors/linkingerror')
  , AssertionFlowNotSupported = require('./errors/assertionflownotsupportederror')
;
...
module.exports = function jwtBearer(options, issue) {
  ...
  return function jwt_bearer(req, res, next) {
    ...
    var client = req[userProperty]
      ...
      , consentCode = req.body['consent_code']
      , separator = '.';
    ...
    function issued(err, accessToken, params) {
      ...
      if (err && err instanceof UserNotFoundError) {
        json = JSON.stringify({
          ...
          error_consent_code: err.consentCode
        });
      ...
      if (err && err instanceof AssertionFlowNotSupported) {
        json = JSON.stringify({
          error: "server_error",
          error_description: "Client does not support assertion flow"
        });
      ...
      if (err && err instanceof LinkingError) {
        json = JSON.stringify({
          error: "token_error",
          error_description: "User already created",
          login_hint: err.login_hint
        });
    ...
    if (arity === 7) {
      ...
      issue(client, contents[0], contents[1], contents[2], intent, consentCode, issued);
    } else if (arity === 6) {
      // contents[0] = header, contents[1] = claimSet, contents[2] = signature
      var data = contents[0] + separator + contents[1];
      issue(client, data, contents[2], intent, consentCode, issued);
    } else { // arity == 5
      issue(client, jwtBearer, intent, consentCode, issued);
    }
  }
}
